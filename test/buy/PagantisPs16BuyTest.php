 ...
 */
class AfterpayPs16BuyTest extends AbstractPs16Selenium
{
    /**
     * config route
     */
    const NOTIFICATION_FOLDER = '/index.php?fc=module&module=afterpay&controller=notify&product=AFTERPAY&key=xxxxxx';
    ...
    {
        $notifyUrl = self::PS16URL.self::NOTIFICATION_FOLDER.'&id_cart=';
     ...
     */
    protected function checkAfterpayOrderId()
    {
        $orderId=0;
    ...
    {
        $notifyUrl = self::PS16URL.self::NOTIFICATION_FOLDER.'&id_cart=6';
        $response = Request::post($notifyUrl)->expects('json')->send();
        $this->assertNotEmpty($response->body->result, $response);
        $this->assertNotEmpty($response->body->status_code, $response);
        $this->assertNotEmpty($response->body->timestamp, $response);
