...
use Afterpay\SeleniumFormUtils\SeleniumHelper;
use Test\AfterpayPrestashopTest;
 ...
 * @package Test\Common
 */
abstract class AbstractPs16Selenium extends AfterpayPrestashopTest
{
    /**
     * @throws \Exception
     */
     ...
     * @throws \Exception
     */
    public function uploadAfterpay()
    {
        ...
        $validatorSearch = WebDriverBy::id('anchorAfterpay');
     ...
     * @throws \Exception
     */
    public function getAfterpayBackOffice()
    {
        $this->webDriver->get(self::PS16URL.self::BACKOFFICE_FOLDER);
        ...
        $verify = WebDriverBy::id('AFTERPAY_PUBLIC_KEY');
    ...
    public function goToCheckout($addressExists = false, $verifySimulator = true)
    {
        ...
        $this->waitUntil($condition);
        $this->assertTrue((bool) $condition);
        $afterpaySimulatorAfterpay = WebDriverBy::className('afterpaySimulatorAfterpay');
        $condition = WebDriverExpectedCondition::presenceOfElementLocated($afterpaySimulatorAfterpay);
        $this->waitUntil($condition);
        $this->assertTrue((bool)$condition);
     ...
     * @throws \Exception
     */
    public function verifyAfterpay()
    {
        $afterpayCheckout = WebDriverBy::className('afterpay-checkout');
        $condition = WebDriverExpectedCondition::visibilityOfElementLocated($afterpayCheckout);
        $this->waitUntil($condition);
        $this->assertTrue((bool) $condition);

        var_dump("Pre title---->".$this->webDriver->getTitle());
        sleep(10);
        // $this->webDriver->findElement($afterpayCheckout)->click();
        $this->webDriver->executeScript('document.querySelector(\'.afterpay-checkout\').click();');
